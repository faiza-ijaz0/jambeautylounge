...
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel';
import Autoplay from "embla-carousel-autoplay";
import { Search, Star, ShoppingCart, Filter, Package, Check, Sparkles, ChevronRight, TrendingUp, Box, DollarSign, RefreshCw, Trash2, MapPin } from 'lucide-react';
import { create } from 'zustand';
import { 
  collection, 
  getDocs, 
...
export default function ProductsPage() {
  ...
  // 👇 PRODUCTS COUNT FOR SELECTED BRANCH
  const branchProductCount = selectedBranch && selectedBranch.id !== 'all' 
    ? products.filter(p => 
        (p.branches && p.branches.includes(selectedBranch.id)) || 
        (p.branchNames && p.branchNames.includes(selectedBranch.name))
      ).length 
    : products.length;
      ...
      <section className="top-16 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-100 py-6 px-4 shadow-lg">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {selectedBranch && selectedBranch.id !== 'all' && (
            <div className="bg-gradient-to-r from-primary/5 to-secondary/5 rounded-2xl p-4 border border-primary/10 mb-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex gap-3">
                  <Badge className="bg-secondary/10 text-secondary border-none px-4 py-2">
                    <Package className="w-3 h-3 mr-2" />
                    {branchProductCount} Products Available
                  </Badge>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div>
              ...
              {selectedBranch && selectedBranch.id !== 'all' && (
                <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
                  ...
                  Showing products available at {selectedBranch.name}
                </p>
              )}
            </div>
          ...
          {filteredAndSortedProducts.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-4xl shadow-sm border border-gray-100">
              ...
              <p className="text-gray-500 font-light mb-8 max-w-md mx-auto">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No products available at ${selectedBranch.name}. Try selecting a different branch.`
                  : 'No products match your current filters. Try adjusting your search criteria.'}
              </p>
              <Button 
                variant="outline" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {filteredAndSortedProducts.map((product) => {
                ...
                const isAvailableInCurrentBranch = selectedBranch && selectedBranch.id !== 'all' 
                  ? (product.branches?.includes(selectedBranch.id) || product.branchNames?.includes(selectedBranch.name))
                  : true;
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      "group bg-white border border-gray-100 transition-all duration-500 p-5 rounded-3xl flex flex-col shadow-[0_8px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] relative",
                      product.totalStock <= 0 || !isAvailableInCurrentBranch
                        ? "opacity-80" 
                        : "hover:border-secondary/20"
                    )}
                  >
                    {/* Out of Stock or Not Available Overlay */}
                    {product.totalStock <= 0 && (
                      <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 rounded-3xl flex items-center justify-center">
                        <Badge className="bg-red-500 text-white border-none px-3 py-1.5 text-[10px] font-black tracking-widest shadow-lg">
                          OUT OF STOCK
                    ...
                    {/* Not Available in Current Branch Overlay */}
                    {product.totalStock > 0 && !isAvailableInCurrentBranch && selectedBranch && selectedBranch.id !== 'all' && (
                      <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 rounded-3xl flex items-center justify-center">
                        <Badge className="bg-amber-500 text-white border-none px-3 py-1.5 text-[10px] font-black tracking-widest shadow-lg">
                          NOT AT THIS BRANCH
                        </Badge>
                      </div>
                    )}
                    ...
                    <div className="flex-1 space-y-2 flex flex-col">
                      ...
                      {product.branchNames && product.branchNames.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {product.branchNames.slice(0, 2).map((branch, index) => (
                            <Badge 
                              key={index} 
                              variant="outline" 
                              className={cn(
                    ...
                    <Button 
                      onClick={() => {
                        if (isInCart) {
                          handleViewCart();
                        } else if (isAvailableInCurrentBranch && product.totalStock > 0) {
                          handleAddToCart(product);
                        } else if (!isAvailableInCurrentBranch && selectedBranch && selectedBranch.id !== 'all') {
                          alert(`This product is not available at ${selectedBranch.name}. Please select a different branch.`);
                        }
                      }}
                      disabled={product.totalStock <= 0 || !isAvailableInCurrentBranch}
                      className={cn(
                        "w-full mt-4 h-11 rounded-xl font-black tracking-[0.2em] text-[9px] transition-all duration-500 shadow-md",
                        isInCart 
                          ? "bg-green-600 hover:bg-green-600 text-white" 
                    ...
                    >
                      ...
                      ) : !isAvailableInCurrentBranch && selectedBranch && selectedBranch.id !== 'all' ? (
                        <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-500">
                        {selectedBranch && selectedBranch.id !== 'all' ? `Products at ${selectedBranch.name}` : 'Total Products'}
                      </p>
                      <p className="text-2xl font-bold text-secondary">
                        {selectedBranch && selectedBranch.id !== 'all' ? branchProductCount : totalProducts}
                      </p>
                    </div>
                  </div>
                </div>
