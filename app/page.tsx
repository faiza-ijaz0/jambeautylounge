...
//                  </div>
//               </div>
//               <Button asChild className="bg-[#FA9DB7] hover:bg-pink-900 text-white font-black rounded-full px-8 py-6 text-[10px] tracking-[0.3em] uppercase shadow-xl hover:scale-105 transition-all duration-500">
...
//                 The Lounge <br /><span className="text-[#FA9DB7] italic">Experience.</span>
//               </h2>
//             </div>
...
//             The <span className="text-[#FA9DB7] italic">Inner</span> Circle
//           </h2>
...
//               Redefine <br />
//               <span className="text-[#FA9DB7] italic">Your Style.</span>
//             </h2>
...
import { db } from '@/lib/firebase';
import { Footer } from '@/components/shared/Footer';
import { useBranch } from "@/contexts/BranchContext"; // 👈 IMPORT ADD KARO
...
const useHomeStore = create<HomeStore>((set, get) => ({
  ...
  fetchCategories: async () => {
    try {
      const categoriesRef = collection(db, 'categories');
      // Simple query se data fetch karo
      const q = query(categoriesRef, orderBy('createdAt', 'desc')); // Limit increase to 20
      const querySnapshot = await getDocs(q);
      ...
      querySnapshot.forEach((doc: QueryDocumentSnapshot<DocumentData>) => {
        ...
        categoriesData.push({
          ...
          name: data.name || 'Unnamed Category',
          description: data.description || 'No description available',
          image: data.image || 'https://images.unsplash.com/photo-1560066984-138dadb4c035?q=80&w=2074&auto=format&fit=crop',
          branchId: data.branchId || '',
          branchName: data.branchName || 'Unknown Branch',
          branchCity: data.branchCity || 'Unknown City',
          type: data.type || 'general',
      ...
      // ✅ Client side filter for active categories
      const activeCategories = categoriesData.filter(cat => cat.isActive);
      set({ categories: activeCategories }); // Sab active categories dikhao
    } catch (error) {
      console.error('Error fetching categories:', error);
      set({ categories: [] });
    }
...
// ==================== MAIN COMPONENT ====================
export default function Home() {
  const { selectedBranch } = useBranch(); // 👈 BRANCH CONTEXT USE KARO
  ...
  const filteredServices = selectedBranch && selectedBranch.id !== 'all'
    ? services.filter(service => 
        (service.branches && service.branches.includes(selectedBranch.id)) || 
        (service.branchNames && service.branchNames.includes(selectedBranch.name))
      )
    : services;

  const filteredProducts = selectedBranch && selectedBranch.id !== 'all'
    ? products.filter(product => 
        (product.branches && product.branches.includes(selectedBranch.id)) || 
        (product.branchNames && product.branchNames.includes(selectedBranch.name))
      )
    : products;

  const filteredStaff = selectedBranch && selectedBranch.id !== 'all'
    ? staff.filter(member => 
        (member.branches && member.branches.includes(selectedBranch.id)) || 
        (member.branchNames && member.branchNames.includes(selectedBranch.name))
      )
    : staff;

  const filteredOffers = selectedBranch && selectedBranch.id !== 'all'
    ? offers.filter(offer => 
        (offer.branches && offer.branches.includes(selectedBranch.id)) || 
        (offer.branchNames && offer.branchNames.includes(selectedBranch.name))
      )
    : offers;

  const filteredMemberships = selectedBranch && selectedBranch.id !== 'all'
    ? memberships.filter(membership => 
        (membership.branches && membership.branches.includes(selectedBranch.id)) || 
        (membership.branchNames && membership.branchNames.includes(selectedBranch.name))
      )
    : memberships;

  const filteredCategories = selectedBranch && selectedBranch.id !== 'all'
    ? categories.filter(category => 
        category.branchId === selectedBranch.id || 
        category.branchName === selectedBranch.name
      )
    : categories;
  ...
  const displayMemberships = filteredMemberships;
  const displayCategories = filteredCategories; // Categories limit 12

  // Stats with filters
  const totalActiveServices = displayServices.filter(s => s.status === 'active').length;
  const totalActiveProducts = displayProducts.filter(p => p.status === 'active').length;
  const totalActiveStaff = displayStaff.filter(s => s.status === 'active').length;
  const totalActiveBranches = branches.filter(b => b.status === 'active').length;
  const totalActiveOffers = displayOffers.length;
  const totalActiveMemberships = displayMemberships.length;
  const totalActiveCategories = displayCategories.length;

  const totalServicesRevenue = displayServices.reduce((sum, service) => sum + service.revenue, 0);
  const totalProductsRevenue = displayProducts.reduce((sum, product) => sum + product.revenue, 0);
  const totalRevenue = totalServicesRevenue + totalProductsRevenue;
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc]">
      ...
      {showChatPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-[60]">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 w-full animate-in fade-in zoom-in duration-300">
            ...
            <Link href="/customer/login" className="block w-full text-center bg-gradient-to-r from-purple-500 to-blue-500 text-white font-semibold py-3 px-6 rounded-xl hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
              Login / Sign Up
            </Link>
      ...
      {/* Categories Slider Section - FIXED with proper carousel */}
      <section className="py-24 px-4 bg-gray-50/30 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <div className="space-y-4">
              ...
              <h2 className="text-5xl md:text-7xl font-serif font-bold text-primary tracking-tight">
                Service Categories
                {selectedBranch && selectedBranch.id !== 'all' && (
                  <span className="text-secondary text-3xl ml-4">at {selectedBranch.name}</span>
                )}
              </h2>
              <p className="text-gray-400 font-light text-lg">Choose your journey through our specialized beauty realms.</p>
            </div>
          ...
          {displayCategories.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
              ...
              <h3 className="text-2xl font-serif font-bold text-gray-400 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No categories at ${selectedBranch.name}`
                  : 'Finding Categories...'}
              </h3>
            </div>
          ) : (
            ...
            >
                  ...
                  <CarouselItem key={category.id} className="pl-4 basis-1/2 md:basis-1/3 lg:basis-1/4">
                    <Link href={`/services?category=${category.name}`}>
                      <div className="group cursor-pointer relative aspect-square overflow-hidden rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-700">
                        ...
                        {selectedBranch && selectedBranch.id !== 'all' && category.branchName === selectedBranch.name && (
                          <Badge className="absolute top-2 right-2 bg-primary text-white text-[8px]">
              ...
              <div className="flex justify-center gap-3 mt-8">
                <CarouselPrevious className="static translate-y-0 w-10 h-10 border-primary/10 hover:bg-primary hover:text-white transition-all" />
                <CarouselNext className="static translate-y-0 w-10 h-10 border-primary/10 hover:bg-primary hover:text-white transition-all" />
              </div>
            </Carousel>
          )}
      ...
      {/* Services Slider Section - UI PEHLE JESA, SIRF SERVICES KI LIMIT HATAI */}
      <section className="py-32 px-4 bg-white relative">
        <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-secondary/5 rounded-full blur-[120px] pointer-events-none"></div>
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          ...
          {displayServices.length === 0 ? (
            <div className="text-center py-24 bg-gray-50/50 rounded-[4rem] border border-dashed border-gray-200">
              <Sparkles className="w-20 h-20 text-gray-200 mx-auto mb-6" />
              <h3 className="text-3xl font-serif font-bold text-gray-400 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No services at ${selectedBranch.name}`
                  : 'Awaiting Excellence'}
              </h3>
              <p className="text-gray-400 font-light">Rituals will appear here once they are prepared.</p>
            </div>
            ...
            <Carousel opts={{ align: "start" }} className="w-full">
              <CarouselContent className="-ml-6">
                {displayServices.map((service) => (
                  <CarouselItem key={service.id} className="pl-6 md:basis-1/2 lg:basis-1/4 xl:basis-1/5">
                    {/* 👇 UI BILKUL PEHLE JESA - KOI CHANGE NAHI */}
                    <Card className="group border-none bg-transparent shadow-none overflow-hidden transition-all duration-700">
                      <div className="relative aspect-4/5 overflow-hidden rounded-4xl shadow-lg group-hover:shadow-2xl transition-all duration-700">
                        <img 
                          src={service.imageUrl || "https://images.unsplash.com/photo-1599351431247-f5094021186d?q=80&w=2070&auto=format&fit=crop"} 
              ...
              {/* 👇 NAVIGATION BUTTONS - PEHLE JAISE HI */}
              <div className="hidden md:flex justify-center gap-4 mt-12">
                <CarouselPrevious className="static translate-y-0 w-12 h-12 border-primary/10 text-primary hover:bg-primary hover:text-white transition-all shadow-lg rounded-full" />
                <CarouselNext className="static translate-y-0 w-12 h-12 border-primary/10 text-primary hover:bg-primary hover:text-white transition-all shadow-lg rounded-full" />
              </div>
      ...
      <section className="py-32 px-4 bg-primary text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
            <div className="space-y-4">
              ...
              <h2 className="text-5xl md:text-7xl font-serif font-bold text-white tracking-tight">
                Couture Skincare
                {selectedBranch && selectedBranch.id !== 'all' && (
                  <span className="text-[#FA9DB7] text-3xl ml-4">at {selectedBranch.name}</span>
                )}
              </h2>
              <p className="text-white/50 font-light text-lg max-w-xl">Scientifically formulated. Artistically packaged. Experience the JAM collection.</p>
            </div>
          ...
          {displayProducts.length === 0 ? (
            <div className="text-center py-24 bg-white/5 rounded-[4rem] border border-dashed border-white/10 backdrop-blur-md">
              <ShoppingBag className="w-20 h-20 text-white/20 mx-auto mb-6" />
              <h3 className="text-3xl font-serif font-bold text-white/60 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No products at ${selectedBranch.name}`
                  : 'Awaiting Inventory'}
              </h3>
              <p className="text-white/50 font-light">Our exclusive collection is being curated.</p>
            </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <div className="space-y-2">
              ...
              <h2 className="text-4xl md:text-5xl font-serif font-bold text-primary">
                Member Rewards
                {selectedBranch && selectedBranch.id !== 'all' && (
                  <span className="text-secondary text-2xl ml-4">at {selectedBranch.name}</span>
                )}
              </h2>
              <Badge variant="outline" className="border-secondary/30 text-secondary mt-2">
                {totalActiveOffers} Active Offers Available
          ...
          {displayOffers.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
              ...
              <h3 className="text-2xl font-serif font-bold text-gray-400 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No offers at ${selectedBranch.name}`
                  : 'No Offers Available'}
              </h3>
              <p className="text-gray-400 font-light">Add active offers </p>
              <Button onClick={fetchHomeData} className="mt-4 bg-secondary hover:bg-secondary/90 text-white">
                <RefreshCw className="w-4 h-4 mr-2" />
                Refresh
              </Button>
            </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          {displayMemberships.length === 0 ? (
            <div className="text-center py-20 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
              ...
              <h3 className="text-2xl font-serif font-bold text-gray-400 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No memberships at ${selectedBranch.name}`
                  : 'No Memberships Available'}
              </h3>
              <p className="text-gray-400 font-light">Add membership plans </p>
              <Button onClick={fetchHomeData} className="mt-4 bg-secondary hover:bg-secondary/90 text-primary">
                <RefreshCw className="w-4 h-4 mr-2" />
                Refresh
              </Button>
            </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-8">
            ...
            <div className="flex flex-col items-end gap-4">
              <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-full border border-gray-100">
                 ...
                 <div className="px-4">
                   <p className="text-[10px] font-black tracking-widest text-primary uppercase">{displayStaff.length} ELITE MASTERS</p>
                   <div className="flex items-center gap-1">
                     <Star className="w-3 h-3 fill-secondary text-secondary" />
                     <span className="text-[10px] font-bold text-primary font-serif italic">4.9/5 Average Rating</span>
                   </div>
          ...
          {displayStaff.length === 0 ? (
            <div className="text-center py-24 bg-gray-50 rounded-[4rem] border border-dashed border-gray-200">
              ...
              <h3 className="text-3xl font-serif font-bold text-gray-400 mb-2">
                {selectedBranch && selectedBranch.id !== 'all' 
                  ? `No staff at ${selectedBranch.name}`
                  : 'Artisans Off-Duty'}
              </h3>
              <p className="text-gray-400 font-light">Our masters are currently preparing for the next season.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10">
              {branches.map((branch) => (
                <div key={branch.id} className={cn(
                  "group cursor-pointer bg-white p-10 rounded-[3rem] border border-transparent hover:border-secondary/20 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] transition-all duration-700 relative overflow-hidden",
                  selectedBranch?.id === branch.id && "ring-2 ring-secondary ring-offset-2"
                )}>
                  <div className="absolute top-0 right-0 w-24 h-24 bg-secondary/5 rounded-bl-[4rem] group-hover:w-full group-hover:h-full group-hover:rounded-none transition-all duration-700"></div>
                  <div className="relative z-10 space-y-8">
                    <div className="w-16 h-16 rounded-3xl bg-gray-50 flex items-center justify-center text-secondary group-hover:bg-white group-hover:scale-110 transition-all duration-700 shadow-sm">
